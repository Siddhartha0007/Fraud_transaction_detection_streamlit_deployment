...
def user_input_features():
    ...
    typeid= st.sidebar.selectbox("Transaction Modes:", data1["type"].unique())
...
def label_analysis():
    ...
    p1=plot1()
    st.write("Distribution of Transctions")
    st.pyplot(p1)
...
# Creating plot
    ...
    p2=plot2()
    st.write("Distributions of Fraud and Not Fraud Transactions")
    st.pyplot(p2)
    def plot3():
    ...
    p3=plot3()
    st.write("Different kind of transactions Types on Not fraud data")
    st.pyplot(p3)
...
#     
#         return data.select_dtypes('float').corr().style.applymap(color_)
#          
#     p6=plot6()
#     #st.write(" distributions of various continuous features")
#     st.pyplot(p6)
...
# =============================================================================
        def header(url):
            st.markdown(f'<p style="background-color:lightblue ;color:white;font-size:20px;border-radius:2%;">{url}</p>', unsafe_allow_html=True)
        html_temp111 = """
		The fraudulent transaction is one of the most serious threats to online security nowadays. 
